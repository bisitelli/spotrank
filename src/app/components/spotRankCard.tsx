    ...
    return (
        <div className="flex flex-col justify-between h-full rounded-xl border border-border-light bg-background-light p-6 shadow-sm transition-shadow">
            ...
            <div className="mt-12">
                <button
                    onClick={() => {
                        if (!ctaLink) return; // tai näytä virheilmoitus
                        window.location.href = ctaLink;
                    }}
                    className="w-full rounded-lg bg-primary text-white font-medium py-2.5 hover:bg-primary/90 transition-colors cursor-pointer"
                >
                    {cta}
                </button>
            </div>
